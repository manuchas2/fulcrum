{%- set peer_id = chaincodepeer['name'] %}
...
vault:
  ...
  secretgitprivatekey: {{ chaincodepeer.repository.gitprivatekeyvaultpath }}
chaincode:
  name: {{ config.name }}
  version: "{{ config.version }}"
  repository:
    {{ chaincodepeer.repository | to_nice_yaml(indent=2) | indent(4) }}
orderer:
